...
@section('content')
    @include('Core::includes.page_header', ['headerTitle' => __('Popup::general.title.create')])
                ...
                <div class="card-body">
                    <form action="{{ route('dawnstar.popups.store') }}" method="POST" id="popupForm">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="form-floating mb-3">
                                    <input class="form-control @error('end_date') is-invalid @enderror" id="end_date"
                                           min="{{ date('Y-m-d') }}"
                                           type="date" name="end_date" value="{{ old('start_date') }}">
                            ...
                            <div class="col-lg-6">
                                <div class="form-floating mb-3">
                                    <select class="form-select @error('display') is-invalid @enderror" id="display" name="display">
                                        @for($i = 1; $i <= 3; $i++)
                                            <option {{ old('start_date', 1) == $i ? 'selected' : '' }} value="{{ $i }}">@lang('Popup::general.display.' . $i)</option>
                                        @endfor
                                    </select>
                            ...
                            <div class="col-lg-6">
                                <div class="form-floating mb-3 d-none">
                                    <select class="select2 form-select select2-multiple" data-toggle="select2" id="urls" name="urls[]" multiple data-placeholder="@lang('Core::general.select')...">
                                        @foreach($urls ?? [] as $url)
                                            <option {{ in_array($url->id, old('urls', [])) ? 'selected' : '' }} value="{{ $url->id }}">{{ $url->url }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-6">
                                <div class="form-floating mb-3">
                                    <input type="number" min="0" id="display_time" class="form-control @error('display_time') is-invalid @enderror" value="{{ old('display_time') }}">
                                    <label for="display_time">@lang('Popup::general.labels.display_time')</label>
                                    @error('display_time')
                                    <div class="invalid-feedback d-block">
                                        {{ $message }}
                                    </div>
